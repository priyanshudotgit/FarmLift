...
<main class="flex min-h-screen w-full pt-20">
    <div class="hidden lg:flex lg:w-1/2 relative bg-surface-container overflow-hidden">
        ...
        <div class="absolute bottom-16 left-12 max-w-lg text-white">
            <h2 class="font-display text-[40px] leading-tight mb-4">Bridging the gap between field and fleet.</h2>
            <p class="font-body-lg text-white/80">Advanced logistics solutions for modern agricultural supply chains.</p>
        </div>
    </div>
    ...
    <div class="w-full lg:w-1/2 flex items-center justify-center relative p-6 pt-24 pb-12 overflow-hidden bg-pattern">
        ...
        <div class="w-full max-w-[440px] z-10">
            <div class="text-center mb-10 relative z-10">
                <h1 class="font-h1 text-[36px] text-primary tracking-tight font-bold">FarmLift</h1>
            ...
            <div class="bg-surface-variant/50 backdrop-blur-md rounded-xl p-1.5 flex mb-8 relative shadow-inner border border-outline-variant/20">
                <button id="login-farmer-btn" class="flex-1 py-2.5 font-label-sm text-[15px] rounded-lg transition-all z-10 flex items-center justify-center gap-2" type="button" onclick="setLoginRole('farmer')">
                    <span class="material-symbols-outlined text-[20px]">agriculture</span>
                    Farmer
                </button>
                <button id="login-driver-btn" class="flex-1 py-2.5 font-label-sm text-[15px] rounded-lg transition-all z-10 flex items-center justify-center gap-2" type="button" onclick="setLoginRole('driver')">
            ...
            <form class="space-y-6 relative z-10" action="{{ route('login') }}" method="POST">
                @csrf
                <input type="hidden" name="role" id="login-role" value="{{ old('role', 'driver') }}" />
                <div>
                    <div class="relative bg-surface/50 rounded-t-lg px-4 pt-5 pb-2 border-b-2 border-outline-variant hover:border-outline focus-within:border-driver-state focus-within:bg-surface/80 transition-all" id="login-email-wrap">
                        <input class="peer w-full bg-transparent font-body-md text-body-md text-on-surface focus:outline-none placeholder-transparent" id="email" name="email" placeholder="Email" required type="email" value="{{ old('email') }}" />
                        <label class="absolute left-4 top-2 font-label-sm text-[12px] text-outline transition-all peer-placeholder-shown:top-3.5 peer-placeholder-shown:text-body-md peer-focus:top-2 peer-focus:text-[12px] peer-focus:text-driver-state cursor-text" for="email" id="login-email-label">Email Address</label>
                    </div>
                    @error('email')<p class="text-error text-[12px] font-label-sm mt-1 ml-1">{{ $message }}</p>@enderror
                </div>
                <div>
                    <div class="relative bg-surface/50 rounded-t-lg px-4 pt-5 pb-2 border-b-2 border-outline-variant hover:border-outline focus-within:border-driver-state focus-within:bg-surface/80 transition-all" id="login-pass-wrap">
                        ...
                        <label class="absolute left-4 top-2 font-label-sm text-[12px] text-outline transition-all peer-placeholder-shown:top-3.5 peer-placeholder-shown:text-body-md peer-focus:top-2 peer-focus:text-[12px] peer-focus:text-driver-state cursor-text" for="password" id="login-pass-label">Password</label>
                    </div>
                    @error('password')<p class="text-error text-[12px] font-label-sm mt-1 ml-1">{{ $message }}</p>@enderror
                </div>
                <div class="flex items-center justify-between mt-4 px-1">
                    <label class="flex items-center gap-3 cursor-pointer group">
                        <input type="checkbox" name="remember" class="w-5 h-5 rounded border-2 border-outline shadow-sm" />
                    ...
                    <a class="font-label-sm text-label-sm text-driver-state hover:text-primary hover:underline font-medium transition-colors" id="login-forgot" href="#">Forgot password?</a>
                </div>
                <div class="pt-8 space-y-4">
                    <button class="w-full py-3.5 px-4 bg-gradient-to-r from-driver-state to-primary text-on-primary font-label-sm text-body-md font-bold rounded-xl hover:shadow-[0_8px_20px_rgba(76,140,228,0.3)] hover:-translate-y-0.5 transition-all flex justify-center items-center gap-2" id="login-submit" type="submit">
                        Sign In
            ...
            <div class="mt-10 text-center relative z-10 pt-6 border-t border-outline-variant/30">
                <p class="font-body-md text-body-md text-on-surface-variant">Don't have an account?
                    <a class="text-driver-state hover:text-primary hover:underline font-bold ml-1 transition-colors" id="login-reg-link" href="{{ route('register') }}">Create Account</a>
                </p>
            </div>
        </div>
    </div>
...
function setLoginRole(role) {
    ...
    var fb = document.getElementById('login-farmer-btn');
    var db = document.getElementById('login-driver-btn');
    var sub = document.getElementById('login-submit');
    var forgot = document.getElementById('login-forgot');
    var regLink = document.getElementById('login-reg-link');
    ...
    if (role === 'farmer') {
        ...
        forgot.className = 'font-label-sm text-label-sm text-farmer-state hover:text-primary hover:underline font-medium transition-colors';
        regLink.className = 'text-farmer-state hover:text-primary hover:underline font-bold ml-1 transition-colors';
    } else {
        ...
        forgot.className = 'font-label-sm text-label-sm text-driver-state hover:text-primary hover:underline font-medium transition-colors';
        regLink.className = 'text-driver-state hover:text-primary hover:underline font-bold ml-1 transition-colors';
    }
}
document.addEventListener('DOMContentLoaded', function() {
    setLoginRole(document.getElementById('login-role').value || 'driver');
});
</script>
@endsection
